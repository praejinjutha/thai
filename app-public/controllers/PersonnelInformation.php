...
class PersonnelInformation extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->load->model('PersonnelInformation_model');
    ...
    public function index()
    {
        ...
        $this->data['Status'] = $this->PersonnelInformation_model->get_Status();
        
        $this->data['view_file'] = 'PersonnelHome/PersonnelInformation/PersonnelInformationHome';
...
// *****************************************  Insert Data Controller  *****************************************  
    public function add()
    {
        $this->data['NStatus'] = $this->PersonnelInformation_model->get_Status();
        $this->data['PName'] = $this->PersonnelInformation_model->get_TSPname();
        $this->data['PEName'] = $this->PersonnelInformation_model->get_STPname();
        // $this->data['PLevel'] = $this->PersonnelInformation_model->get_OdrLev();
        $this->data['PoType'] = $this->PersonnelInformation_model->get_TeacherType();
        $this->data['Subject_group'] = $this->PersonnelInformation_model->get_SubjectGroup();
        $PosName = $this->PersonnelInformation_model->get_Position();
        $this->data["PosName"] = $PosName->result();

        // retuen data and redirect to page
        $this->data['view_file'] = 'PersonnelHome/PersonnelInformation/PersonnelInformationAdd';
        $this->load->view(THEMES, $this->data);
    }

    function fetch_TeacherPosition()   
    {
       echo $this->PersonnelInformation_model->fetch_TeacherPosition($this->input->post('PoType'));
    }

    function fetch_TeacherPoLevel()   
    {
       echo $this->PersonnelInformation_model->fetch_TeacherPoLevel($this->input->post('PoLine'));
    }
    ...
    { 
        ...
        $results = $this->PersonnelInformation_model->Check_IDCard($IDCard)->num_rows();
        ...
        if($results >= 1){
            ...
            if(!isset($_FILES['Pic']) || $_FILES['Pic']['error'] == UPLOAD_ERR_NO_FILE) {
                ...
                $results = $this->PersonnelInformation_model->insert_Data($MenuStaData, $MoneyData,$PicAll);
    ...
    {   
        $this->session->set_userdata('idcard_id', $IDCard);
        $IDCard = $this->session->userdata('idcard_id');

        $ResultData = $this->PersonnelInformation_model->get_PersonnelData($IDCard);
        foreach ($ResultData->result() as $row) {
            ...
            $this->data['NSalary'] = ($this->data['MSalary'] * 1.03) - $this->data['MSalary'];
        ...
        $ResultAddress = $this->PersonnelInformation_model->get_PersonnelAddress($IDCard);
        foreach ($ResultAddress->result() as $row) {
        ...
        $this->data['view_file'] = 'PersonnelHome/PersonnelInformation/PersonnelInformationShow';
        $this->load->view(THEMES, $this->data);
    }
...
// *****************************************  Family Controller  *****************************************     
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelFamily($this->session->userdata('idcard_id'))->result();
...
// *****************************************  Study Controller  *******************************************
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelStudy($this->session->userdata('idcard_id'))->result();
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelFame($this->session->userdata('idcard_id'))->result();
    ...
    public function export($IDCard, $P_at){
        $result = $this->PersonnelInformation_model->export_File_Fame($IDCard, $P_at)->result();
...
// *****************************************  Training Controller  *******************************************
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelTraining($this->session->userdata('idcard_id'))->result();
...
// *****************************************  Position Controller  *******************************************
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelPosition($this->session->userdata('idcard_id'))->result();
...
// *****************************************  Teaching Controller  *******************************************
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelTeaching($this->session->userdata('idcard_id'))->result();
...
// *****************************************  License Controller  *******************************************
    public function get_PersonnelLicense()
        {
            $data = $this->PersonnelInformation_model->get_PersonnelLicense($this->session->userdata('idcard_id'))->result();
...
// *****************************************  Leave Controller  *******************************************
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelLeave($this->session->userdata('idcard_id'))->result();
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelRename($this->session->userdata('idcard_id'))->result();
...
// *****************************************  Insignia Controller  *******************************************
    ...
    {
        $data = $this->PersonnelInformation_model->get_PersonnelInsignia($this->session->userdata('idcard_id'))->result();
    ...
    public function Export_Personnel()
    {
        ...
        $studySheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE)->setPaperSize(PageSetup::PAPERSIZE_A4);
        ...
        $TrainingSheet = $spreadsheet->createSheet();
        $TrainingSheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE)->setPaperSize(PageSetup::PAPERSIZE_A4);
        $TrainingSheet->getPageMargins()
            ->setTop(0.5)
            ->setRight(0.5)
            ->setBottom(0.5)
            ->setLeft(0.5)
            ->setHeader(0.8)
            ->setFooter(0.8);
        $TrainingSheet->setTitle('ข้อมูลประวัติการศึกษา อบรมดูงาน');

        $TeachingSheet = $spreadsheet->createSheet();
        $TeachingSheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE)->setPaperSize(PageSetup::PAPERSIZE_A4);
        $TeachingSheet->getPageMargins()
            ->setTop(0.5)
            ->setRight(0.5)
            ->setBottom(0.5)
            ->setLeft(0.5)
            ->setHeader(0.8)
            ->setFooter(0.8);
        $TeachingSheet->setTitle('ข้อมูลการสอน');

        $LicenseSheet = $spreadsheet->createSheet();
        $LicenseSheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE)->setPaperSize(PageSetup::PAPERSIZE_A4);
        $LicenseSheet->getPageMargins()
            ->setTop(0.5)
            ->setRight(0.5)
            ->setBottom(0.5)
            ->setLeft(0.5)
            ->setHeader(0.8)
            ->setFooter(0.8);
        $LicenseSheet->setTitle('ข้อมูลใบอนุญาตประกอบวิชาชีพ');

        $LeaveSheet = $spreadsheet->createSheet();
        $LeaveSheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE)->setPaperSize(PageSetup::PAPERSIZE_A4);
        $LeaveSheet->getPageMargins()
            ->setTop(0.5)
            ->setRight(0.5)
            ->setBottom(0.5)
            ->setLeft(0.5)
            ->setHeader(0.8)
            ->setFooter(0.8);
        $LeaveSheet->setTitle('ข้อมูลประวัติการลาประจำปี');

        $InsigniaSheet = $spreadsheet->createSheet();
        $InsigniaSheet->getPageSetup()->setOrientation(PageSetup::ORIENTATION_LANDSCAPE)->setPaperSize(PageSetup::PAPERSIZE_A4);
        $InsigniaSheet->getPageMargins()
            ->setTop(0.5)
            ->setRight(0.5)
            ->setBottom(0.5)
            ->setLeft(0.5)
            ->setHeader(0.8)
            ->setFooter(0.8);
        $InsigniaSheet->setTitle('ข้อมูลประวัติการรับเครื่องราชฯ');
...
//-------------------------------------------- END ข้อมูลครอบครัว ------------------------------------------------------   
        $columnHeadersStudy = [
            'A' => ['label' => 'ปีที่เริ่มศึกษา', 'width' => 9.8],
            'B' => ['label' => 'ปีที่สำเร็จการศึกษา', 'width' => 14],
            'C' => ['label' => 'ว/ด/ป ที่สำเร็จการศึกษา', 'width' => 18.8],
            'D' => ['label' => 'ระดับการศึกษา', 'width' => 11.8],
            'E' => ['label' => 'วุฒิการศึกษา', 'width' => 11.8],
            'F' => ['label' => 'วิชาเอก', 'width' => 11],
            'G' => ['label' => 'วิชาโท', 'width' => 11],
            'H' => ['label' => 'สถาบันการศึกษา', 'width' => 20],
            'I' => ['label' => 'ประเภททุน', 'width' => 12],
            'J' => ['label' => 'หน่วยงานที่ให้', 'width' => 15.8]
        ];

        foreach ($columnHeadersStudy as $column => $headerInfo) {
            $cell = $column . '4';

            $studySheet->getStyle($cell)->applyFromArray([
                'alignment' => [
                    'horizontal' => Alignment::HORIZONTAL_CENTER,
                    'vertical' => Alignment::VERTICAL_CENTER,
                ],
            ...
            $studySheet->setCellValue($cell, $headerInfo['label']);
            $studySheet->getColumnDimension($column)->setWidth($headerInfo['width']);
            $studySheet->getRowDimension(4)->setRowHeight(25);
        }

        $studySheet->mergeCells('A2:J2');
        ...
        $ResultStudy = $this->PersonnelInformation_model->get_PersonnelStudyExport($IDCard)->result();

        $startRow = 5;

        foreach ($ResultStudy as $row) {
            $data = [
                'A' => ['position' => 'center', 'value' => $row->EStartYear],
                'B' => ['position' => 'left', 'value' => $row->EFinishYear],
                'C' => ['position' => 'center', 'value' => date('d-m-Y', strtotime($row->EFinishDate))],
                'D' => ['position' => 'center', 'value' => $row->BA],
                'E' => ['position' => 'center', 'value' => $row->Degree],
                'F' => ['position' => 'center', 'value' => $row->Major],
                'G' => ['position' => 'center', 'value' => $row->Minor],
                'H' => ['position' => 'center', 'value' => $row->Institute],
                'I' => ['position' => 'center', 'value' => $row->TypeSch],
                'J' => ['position' => 'center', 'value' => $row->CountrySch],
            ];
            foreach ($data as $column => $value) {
                $cell = $column . $startRow;
                $studySheet->getStyle($cell)->applyFromArray([
                ...
                $studySheet->setCellValue($cell, $value['value']);
            }
            $startRow++;
        }
        ...
        $studySheet->mergeCells('A14:J14');
        $studySheet->setCellValue('A14', 'ประวัติการเลื่อนตำแหน่ง');
        $studySheet->getStyle('A14')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $studySheet->getStyle('A14')->getFont()->setBold(true)->setSize(18)->setName('TH SarabunPSK');

        $studySheet->setCellValue('A16', 'ลำดับ')->getStyle('A16')->applyFromArray($StudyHead);
        
        $studySheet->mergeCells('B16:C16');
        $studySheet->setCellValue('B16', 'ตำแหน่ง')->getStyle('B16:C16')->applyFromArray($StudyHead);

        $studySheet->mergeCells('D16:G16');
        $studySheet->setCellValue('D16', 'หน่วยงาน')->getStyle('D16:G16')->applyFromArray($StudyHead);
       
        $studySheet->setCellValue('H16', 'จังหวัด')->getStyle('H16')->applyFromArray($StudyHead);

        $studySheet->mergeCells('I16:J16');
        $studySheet->setCellValue('I16', 'ว/ด/ป การเลื่อนตำแหน่ง')->getStyle('I16:J16')->applyFromArray($StudyHead);

        $ResultPosition = $this->PersonnelInformation_model->get_PersonnelPositionExport($IDCard)->result();
        
        $startRow = 17;

        foreach ($ResultPosition as $row) {
            $data = [
                'A' => ['position' => 'center', 'value' => $row->PLevel],
                'B' => ['position' => 'left', 'value' => $row->PPo],
                'D' => ['position' => 'center', 'value' => $row->PAgencies],
                'H' => ['position' => 'center', 'value' => $row->PProvince],
                'I' => ['position' => 'center', 'value' => $row->PChangeDate]
            ];
            foreach ($data as $column => $value) {
                $cell = $column . $startRow;
                $studySheet->getStyle($cell)->applyFromArray([
            ...
            $studySheet->mergeCells('B' . $startRow . ':C' . $startRow)->getStyle('B' . $startRow . ':C' . $startRow)->applyFromArray($StudyBody);
            $studySheet->mergeCells('D' . $startRow . ':G' . $startRow)->getStyle('D' . $startRow . ':G' . $startRow)->applyFromArray($StudyBody);
            $studySheet->mergeCells('I' . $startRow . ':J' . $startRow)->getStyle('I' . $startRow . ':J' . $startRow)->applyFromArray($StudyBody);

            $startRow++;
        }

//-------------------------------------------- END ข้อมูลการศึกษา การเลื่อนตำแหน่ง ------------------------------------------------------
        $TrainingSheet->mergeCells('A2:J2');
        $TrainingSheet->setCellValue('A2', 'ข้อมูลประวัติการศึกษา อบรม ดูงาน');
        $TrainingSheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $TrainingSheet->getStyle('A2')->getFont()->setBold(true)->setSize(18)->setName('TH SarabunPSK');
        
        $columnHeadersTraining = [
            'A' => ['label' => 'ชื่อหลักสูตรอบรม', 'width' => 25.8],
            'B' => ['label' => 'กลุ่มหลักสูตร', 'width' => 15.8],
            'C' => ['label' => 'วันที่เริ่ม', 'width' => 10.8],
            'D' => ['label' => 'วันที่สิ้นสุด', 'width' => 10.8],
            'E' => ['label' => 'สถานที่', 'width' => 20.8],
            'F' => ['label' => 'จังหวัด', 'width' => 12.8],
            'G' => ['label' => 'ประเทศ', 'width' => 12.8],
            'H' => ['label' => 'วัน', 'width' => 6.8],
            'I' => ['label' => 'ชั่วโมง', 'width' => 6.8],
            'J' => ['label' => 'ค่าใช้จ่าย', 'width' => 11.8]
        ];

        foreach ($columnHeadersTraining as $column => $headerInfo) {
            $cell = $column . '4';

            $TrainingSheet->getStyle($cell)->applyFromArray([
                'alignment' => [
                    'horizontal' => Alignment::HORIZONTAL_CENTER,
                    'vertical' => Alignment::VERTICAL_CENTER,
                ],
            ...
            $TrainingSheet->setCellValue($cell, $headerInfo['label']);
            $TrainingSheet->getColumnDimension($column)->setWidth($headerInfo['width']);
            $TrainingSheet->getRowDimension(4)->setRowHeight(25);
        }

        $ResultTraining = $this->PersonnelInformation_model->get_PersonnelTrainingExport($IDCard)->result();

        $startRow = 5;

        foreach ($ResultTraining as $row) {
            $data = [
                'A' => ['position' => 'center', 'value' => $row->TNameCourse],
                'B' => ['position' => 'left', 'value' => $row->TGCourse],
                'C' => ['position' => 'center', 'value' => date('d-m-Y', strtotime($row->TStartDate))],
                'D' => ['position' => 'center', 'value' => date('d-m-Y', strtotime($row->TFinishDate))],
                'E' => ['position' => 'center', 'value' => $row->TPlace],
                'F' => ['position' => 'center', 'value' => $row->TProvince],
                'G' => ['position' => 'center', 'value' => $row->TCountry],
                'H' => ['position' => 'center', 'value' => $row->TCDay],
                'I' => ['position' => 'center', 'value' => $row->TCHou],
                'J' => ['position' => 'center', 'value' => $row->TPay],
            ];
            foreach ($data as $column => $value) {
                $cell = $column . $startRow;
                $TrainingSheet->getStyle($cell)->applyFromArray([
                ...
                $TrainingSheet->setCellValue($cell, $value['value']);
            }
            $startRow++;
        }
//-------------------------------------------- END ข้อมูลประวัติการศึกษา อบรม ดูงาน ------------------------------------------------------
        $TeachingSheet->mergeCells('A2:I2');
        $TeachingSheet->setCellValue('A2', 'ข้อมูลการสอน');
        $TeachingSheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $TeachingSheet->getStyle('A2')->getFont()->setBold(true)->setSize(18)->setName('TH SarabunPSK');

        $columnHeadersTeaching = [
            'A' => ['label' => 'ปีการศึกษา', 'width' => 10.8],
            'B' => ['label' => 'ภาคเรียนที่', 'width' => 10.8],
            'C' => ['label' => 'สอนตรงตามวิชาเอก', 'width' => 16.8],
            'D' => ['label' => 'กลุ่มสาระการเรียนรู้', 'width' => 16.8],
            'E' => ['label' => 'วิชาที่สอน', 'width' => 16.8],
            'F' => ['label' => 'เป็นวิชาที่ต้องการเข้ารับการพัฒนา', 'width' => 25.8],
            'G' => ['label' => 'ระดับชั้นที่สอน', 'width' => 15.8],
            'H' => ['label' => 'ห้อง', 'width' => 5.8],
            'I' => ['label' => 'ชั่วโมงสอน/สัปดาห์', 'width' => 15.8]
        ];

        foreach ($columnHeadersTeaching as $column => $headerInfo) {
            $cell = $column . '4';

            $TeachingSheet->getStyle($cell)->applyFromArray([
                'alignment' => [
                    'horizontal' => Alignment::HORIZONTAL_CENTER,
                    'vertical' => Alignment::VERTICAL_CENTER,
                ],
            ...
            $TeachingSheet->setCellValue($cell, $headerInfo['label']);
            $TeachingSheet->getColumnDimension($column)->setWidth($headerInfo['width']);
            $TeachingSheet->getRowDimension(4)->setRowHeight(25);
        }

        $ResultTeaching = $this->PersonnelInformation_model->get_PersonnelTeachingExport($IDCard)->result();

        $startRow = 5;

        foreach ($ResultTeaching as $row) {
            $TeaMajor = $row->TeaMajor === 'Y' ? 'สอนตรง' : 'สอนไม่ตรง';
            $DevelopSub = $row->DevelopSub === 'Y' ? 'ใช่' : 'ไม่ใช่';
            $LevClss = $row->Lev . '.' . $row->Clss;
            $data = [
                'A' => ['position' => 'center', 'value' => $row->EYea],
                'B' => ['position' => 'left', 'value' => $row->ETerm],
                'C' => ['position' => 'center', 'value' => $TeaMajor],
                'D' => ['position' => 'center', 'value' => $row->GLearn],
                'E' => ['position' => 'center', 'value' => $row->Titles],
                'F' => ['position' => 'center', 'value' => $DevelopSub],
                'G' => ['position' => 'center', 'value' => $LevClss],
                'H' => ['position' => 'center', 'value' => $row->Room],
                'I' => ['position' => 'center', 'value' => $row->HourPerWeek]
            ];
            foreach ($data as $column => $value) {
                $cell = $column . $startRow;
                $TeachingSheet->getStyle($cell)->applyFromArray([
                ...
                $TeachingSheet->setCellValue($cell, $value['value']);
            }
            $startRow++;
        }
//-------------------------------------------- END ข้อมูลการสอน ------------------------------------------------------
        $LicenseSheet->mergeCells('A2:H2');
        $LicenseSheet->setCellValue('A2', 'ข้อมูลใบอนุญาตประกอบวิชาชีพ');
        $LicenseSheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $LicenseSheet->getStyle('A2')->getFont()->setBold(true)->setSize(18)->setName('TH SarabunPSK');

        $columnHeadersLicense = [
            'A' => ['label' => 'เลขที่ใบประกอบวิชาชีพ', 'width' => 17.8],
            'B' => ['label' => 'ชื่อใบประกอบวิชาชีพ', 'width' => 16.8],
            'C' => ['label' => 'หน่วยงานที่ออกให้', 'width' => 20.8],
            'D' => ['label' => 'ว/ด/ป ที่ออก', 'width' => 14.8],
            'E' => ['label' => 'ว/ด/ป ปีที่หมดอายุ', 'width' => 14.8],
            'F' => ['label' => 'เลขที่ใบอนุญาตทำงาน', 'width' => 16.8],
            'G' => ['label' => 'การต่อใบอนุญาติ', 'width' => 15.8],
            'H' => ['label' => 'การเพิกถอนใบอนุญาติ', 'width' => 17.7]
        ];

        foreach ($columnHeadersLicense as $column => $headerInfo) {
            $cell = $column . '4';

            $LicenseSheet->getStyle($cell)->applyFromArray([
                'alignment' => [
                    'horizontal' => Alignment::HORIZONTAL_CENTER,
                    'vertical' => Alignment::VERTICAL_CENTER,
                ],
            ...
            $LicenseSheet->setCellValue($cell, $headerInfo['label']);
            $LicenseSheet->getColumnDimension($column)->setWidth($headerInfo['width']);
            $LicenseSheet->getRowDimension(4)->setRowHeight(25);
        }

        $ResultLicense = $this->PersonnelInformation_model->get_PersonnelLicenseExport($IDCard)->result();

        $startRow = 5;

        foreach ($ResultLicense as $row) {
            $data = [
                'A' => ['position' => 'center', 'value' => $row->LPNo],
                'B' => ['position' => 'left', 'value' => $row->LPName],
                'C' => ['position' => 'center', 'value' => $row->LPLeave],
                'D' => ['position' => 'center', 'value' => date('d-m-Y', strtotime($row->LPLeaveDate))],
                'E' => ['position' => 'center', 'value' => date('d-m-Y', strtotime($row->LPExpireDate))],
                'F' => ['position' => 'center', 'value' => $row->LPWorkPermit],
                'G' => ['position' => 'center', 'value' => $row->LPToPermit],
                'H' => ['position' => 'center', 'value' => $row->LPRevocation]
            ];
            foreach ($data as $column => $value) {
                $cell = $column . $startRow;
                $LicenseSheet->getStyle($cell)->applyFromArray([
                ...
                $LicenseSheet->getCell($cell)->setValueExplicit($value['value'], DataType::TYPE_STRING);
            }
            $startRow++;
        }
//-------------------------------------------- END ข้อมูลใบอนุญาตประกอบวิชาชีพ ------------------------------------------------------

        $LeaveSheet->mergeCells('A2:J2');
        $LeaveSheet->setCellValue('A2', 'ข้อมูลประวัติการลาประจำปี');
        $LeaveSheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $LeaveSheet->getStyle('A2')->getFont()->setBold(true)->setSize(18)->setName('TH SarabunPSK');

        // $columnHeadersLeave = [
        //     'A' => ['label' => 'ปี', 'width' => 17.8],
        //     'B' => ['label' => 'มาสาย', 'width' => 16.8],
        //     'C' => ['label' => 'ลาป่วย', 'width' => 20.8],
        //     'D' => ['label' => 'ลากิจ', 'width' => 14.8],
        //     'E' => ['label' => 'ลาพักผ่อน', 'width' => 15.8],
        //     'F' => ['label' => 'ลาคลอด', 'width' => 17.7],
        //     'G' => ['label' => 'ลาติดตามคู่สมรส', 'width' => 16.8],
        //     'H' => ['label' => 'ลาบวช/ฮัจช์', 'width' => 17.7],
        //     'I' => ['label' => 'ลาเข้ารับการเตรียมพล', 'width' => 17.7],
        //     'J' => ['label' => 'ลาศึกษาต่อ', 'width' => 17.7],
        //     'K' => ['label' => 'ลาไปปฏิบัติงานในองค์การระหว่างประเทศ', 'width' => 16.8],
        //     'L' => ['label' => 'ขาด', 'width' => 14.8]
        // ];

        $columnHeadersLeave = [
            'A' => ['label' => 'ปี', 'width' => 10],
            'B' => ['label' => 'มาสาย', 'width' => 14],
            'C' => ['label' => 'ลาป่วย', 'width' => 14],
            'D' => ['label' => 'ลากิจ', 'width' => 14],
            'E' => ['label' => 'ขาด', 'width' => 14],
            'F' => ['label' => 'ไปราชการ', 'width' => 14],
            'G' => ['label' => 'ลาพักผ่อน', 'width' => 14],
            'H' => ['label' => 'ลาคลอด', 'width' => 14],
            'I' => ['label' => 'ลาบวช/ฮัจช์', 'width' => 14],
            'J' => ['label' => 'ลาศึกษาต่อ', 'width' => 14]
        ];

        foreach ($columnHeadersLeave as $column => $headerInfo) {
            $cell = $column . '4';

            $LeaveSheet->getStyle($cell)->applyFromArray([
                'alignment' => [
                    'horizontal' => Alignment::HORIZONTAL_CENTER,
                    'vertical' => Alignment::VERTICAL_CENTER,
                ],
            ...
            $LeaveSheet->setCellValue($cell, $headerInfo['label']);
            $LeaveSheet->getColumnDimension($column)->setWidth($headerInfo['width']);
            $LeaveSheet->getRowDimension(4)->setRowHeight(25);
        }

        $ResultLeave = $this->PersonnelInformation_model->get_PersonnelLeaveExport($IDCard)->result();

        $startRow = 5;

        foreach ($ResultLeave as $row) {
            $data = [
                'A' => ['position' => 'center', 'value' => $row->EYa],
                'B' => ['position' => 'left', 'value' => $row->VLate],
                'C' => ['position' => 'center', 'value' => $row->Vsick],
                'D' => ['position' => 'center', 'value' => $row->Vkit],
                'E' => ['position' => 'center', 'value' => $row->VAbsence],
                'F' => ['position' => 'center', 'value' => $row->VServ],
                'G' => ['position' => 'center', 'value' => $row->VRelax],
                'H' => ['position' => 'center', 'value' => $row->VSpawn],
                'I' => ['position' => 'center', 'value' => $row->VOrdinate],
                'J' => ['position' => 'center', 'value' => $row->VStudy]
            ];
            foreach ($data as $column => $value) {
                $cell = $column . $startRow;
                $LeaveSheet->getStyle($cell)->applyFromArray([
                ...
                $LeaveSheet->setCellValue($cell, $value['value']);
            }
            $startRow++;
        }
//-------------------------------------------- END ข้อมูลประวัติการลาประจำปี ------------------------------------------------------

        $InsigniaSheet->mergeCells('A2:H2');
        $InsigniaSheet->setCellValue('A2', 'ข้อมูลประวัติการรับเครื่องราชอิสริยาภรณ์');
        $InsigniaSheet->getStyle('A2')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $InsigniaSheet->getStyle('A2')->getFont()->setBold(true)->setSize(18)->setName('TH SarabunPSK');

        $columnHeadersInsignia = [
            'A' => ['label' => 'ปีที่ได้รับ', 'width' => 10.8],
            'B' => ['label' => 'ชั้นเครื่องราชฯ', 'width' => 15.8],
            'C' => ['label' => 'ตำแหน่ง', 'width' => 29.8],
            'D' => ['label' => 'ราชกิจเล่มที่', 'width' => 15.8],
            'E' => ['label' => 'ราชกิจตอนที่', 'width' => 15.8],
            'F' => ['label' => 'ราชกิจหน้าที่', 'width' => 15.8],
            'G' => ['label' => 'ราชกิจเลขที่', 'width' => 15.8],
            'H' => ['label' => 'ราชกิจวันที่', 'width' => 15.8]
        ];

        foreach ($columnHeadersInsignia as $column => $headerInfo) {
            $cell = $column . '4';

            $InsigniaSheet->getStyle($cell)->applyFromArray([
                'alignment' => [
                    'horizontal' => Alignment::HORIZONTAL_CENTER,
                    'vertical' => Alignment::VERTICAL_CENTER,
                ],
            ...
            $InsigniaSheet->setCellValue($cell, $headerInfo['label']);
            $InsigniaSheet->getColumnDimension($column)->setWidth($headerInfo['width']);
            $InsigniaSheet->getRowDimension(4)->setRowHeight(25);
        }

        $ResultInsignia = $this->PersonnelInformation_model->get_PersonnelInsigniaExport($IDCard)->result();

        $startRow = 5;

        foreach ($ResultInsignia as $row) {
            $data = [
                'A' => ['position' => 'center', 'value' => $row->RRDare],
                'B' => ['position' => 'left', 'value' => $row->RLevel],
                'C' => ['position' => 'center', 'value' => $row->RPo],
                'D' => ['position' => 'center', 'value' => $row->RGVol],
                'E' => ['position' => 'center', 'value' => $row->RGPart],
                'F' => ['position' => 'center', 'value' => $row->RGPage],
                'G' => ['position' => 'center', 'value' => $row->RGNo],
                'H' => ['position' => 'center', 'value' => date('d-m-Y', strtotime($row->RDate))]
            ];
            foreach ($data as $column => $value) {
                $cell = $column . $startRow;
                $InsigniaSheet->getStyle($cell)->applyFromArray([
                ...
                $InsigniaSheet->setCellValue($cell, $value['value']);
            }
            $startRow++;
        }
        
//-------------------------------------------- END ข้อมูลประวัติการรับเครื่องราชอิสริยาภรณ์ ------------------------------------------------------
