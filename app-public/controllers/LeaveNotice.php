...
class LeaveNotice extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->load->model('LeaveNotice_model');
    ...
    {
        $FullName = $this->LeaveNotice_model->get_FullName();
        $this->data["FullName"] = $FullName->result();

        $PosName = $this->LeaveNotice_model->get_PosName()->result();
        $this->data["PosName"] = $PosName;

        $type_leave = $this->LeaveNotice_model->get_AbsentType()->result();
        ...
        $this->data['view_file'] = 'PersonnelHome/LeaveNotice/LeaveNoticeHome';
        $this->load->view(THEMES, $this->data);
    }

    public function user()
    {
        $this->data['view_file'] = 'PersonnelHome/LeaveNotice/LeaveNoticeUser';
        $this->load->view(THEMES, $this->data);
    }

    function fech_LeaveNotice()
    {   
        $Year = $this->input->post('Year');
        $Absent = $this->input->post('Absent');

        if($Year == NULL && $Absent == NULL){
            $results = $this->LeaveNotice_model->fech_LeaveNotice()->result();
        }elseif($Year != NULL && $Absent == NULL){
            $results = $this->LeaveNotice_model->fech_LeaveNoticeYear($Year)->result();
        }elseif($Year == NULL && $Absent != NULL){
            $results = $this->LeaveNotice_model->fech_LeaveNoticeAbsents($Absent)->result();
        }elseif($Year != NULL && $Absent != NULL){
            $results = $this->LeaveNotice_model->fech_LeaveNoticeAll($Year, $Absent)->result();
        }
