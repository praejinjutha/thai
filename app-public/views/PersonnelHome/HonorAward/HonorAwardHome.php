                                ...
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="col-md-2 mt-4">
                                            ...
                                            <select class="form-select" id="Position" name="Position" onclick="removeOption_HonorAward()">
                                            </select>
                                            <hr>
                                            <!-- **** Modal **** -->
...
function get_Position() {
    ...
    $.ajax({
        url: "<?= site_url('HonorAward/fetch_Position') ?>",
        method: "POST",
        data: {
            EdYear: EdYear
        },
...
function get_HonorAward() {
    ...
    $.ajax({
        url: "<?= site_url('HonorAward/fech_HonorAward') ?>",
        method: "POST",
        data: { 
            EdYear: EdYear,
            Position: Position
            ...
            if (data.length === 0) {
                ...
                $.each(data, function(index, row) {
                    ...
                    let table_row = `<tr>
                            ...
                            <td><a href="<?= site_url('HonorAward/export/') ?>${row.ID}">${row.FileNm || ''}</a></td>
                                        ...
                                        <div class="modal-content">
                                            <div class="modal-body" style="text-align: left;">
                                                <div class="tab">
                                                                    <h5 class="card-title fw-bold"
                                                                        style="color: #18409e;font-size: 32px;">
                                                                        แก้ไขข้อมูลการยกย่องเชิดชูเกียรติ / รางวัลเกียรติยศ
                                                                    </h5>
                                                                </div>
                                                                <div id="modal-setting-7" class="tabcontent">
                                                                    <hr style="color: #eac178;height: 3px;opacity: 1;">
                                                                        ...
                                                                        <div class="row">
                                                                            ...
                                                                            <div class="col">
                                                                                <div class="form-group mt-2" id="FileNameDownload${count}">
                                                                                    ...
                                                                                    <a href="<?= site_url('HonorAward/export/') ?>${row.ID}"><input class="form-control" type="text" id="FileNameEdit${count}" name="FileNameEdit" value="${row.FileNm !== null ? row.FileNm : ''}" style="cursor: pointer; color: blue;" disabled></a>
                                                                                </div>
                            ...
                            <td><a href="<?= site_url('HonorAward/Export_Data/')?>${row.ID}" class="btn btn-primary fw-bold fs-4">ส่งออก</a></td>
                        </tr>`;
                    table_body.append(table_row);
                });
            }
...
function InsertHonorAward() {
    ...
    $.ajax({
        url: "<?= site_url('HonorAward/Insert_HonorAward') ?>",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function UpdateHonorAward(ID, modalCount) {
    ...
    $.ajax({
        url: "<?= site_url('HonorAward/update_HonorAward') ?>",
        method: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function DeleteHonorAward(ID) {
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= site_url('HonorAward/delete_HonorAward') ?>",
                type: 'POST',
                data: {
                    ID: ID
                },
