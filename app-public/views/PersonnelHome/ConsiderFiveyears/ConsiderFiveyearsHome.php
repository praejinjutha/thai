                                    ...
                                    <div class="row">
                                        <div class="col-md-3" style="padding-right: 0px">
                                            <label for="exampleInputUsername2" class=" fw-bold"
                                                style="font-size: 26px;">ตำแหน่งงาน</label>
                                            <select class="form-select "
                                                style="display: inline; margin: 0 20px 0 20px;  width: 70%;"
                                                name="Position" id="Position" onclick="removeOption_ConFiveyears()">
                                        ...
                                        <div class="col-md-5">
                                            <label for="exampleInputUsername2" class=" fw-bold"
                                                style="font-size: 26px;">รายชื่อบุคลากร</label>
                                            <select class="form-select "
                                        ...
                                        <div class="col-md-4" style="padding-left: 0px;">
                                            <a href="#" class="btn btn-success fw-bold fs-4 float-end"
                                                ...
                                                style="margin-right: 10px;">+ เพิ่มข้อมูลรายละเอียดการพิจารณา</a>
                                        </div>
                                    </div>
                                    <div class="row my-4">
...
function get_ConsiderFiveyears() {
    ...
    $.ajax({
        url: "<?= site_url('ConsiderFiveyears/fech_ConsiderFiveyears') ?>",
        method: "POST",
        data: {
            Position: Position,
            IDCard: IDCard
            ...
            if (data.length === 0) {
                ...
                $.each(data, function(index, row) {
                    ...
                    let table_row = `<tr>
                            ...
                            <td><a href="<?= site_url('ConsiderFiveyears/export/') ?>${row.id}">${row.Ref || ''}</a></td>
                                                ...
                                                <div class="modal-content">
                                                    <div class="modal-body" style="text-align: left">
                                                        <div class="tab">
                                                            <h5 class="card-title fw-bold"
                                                                style="color: #18409e;font-size: 32px;">
                                                                แก้ไขข้อมูลรายละเอียดการพิจารณาความดีความชอบย้อนหลัง 5
                                                                ปี</h5>
                                                        </div>
                                                                    ...
                                                                    <div class="col">
                                                                        <div class="col">
                                                                            <div class="form-group mt-2" id="FileNameDownload${count}">
                                                                                ...
                                                                                <a href="<?= site_url('ConsiderFiveyears/export/') ?>${row.id}"><input class="form-control" type="text" id="Ref_Edit${count}" name="Ref_Edit" value="${row.Ref !== null ? row.Ref : ''}" style="cursor: pointer; color: blue;" disabled></a>
                                                                            </div>
                            ...
                            <td><a href="<?= site_url('ConsiderFiveyears/Export_Data/')?>${row.id}" class="btn btn-primary fw-bold fs-4">ส่งออก</a></td>
                        </tr>`;
                    table_body.append(table_row);
                });
            }
...
$('#SFullname').change(fetch_IDCard);

function fetch_IDCard() {
    let IDCard = $('#SFullname').val();

    $.ajax({
        url: "<?= site_url('ConsiderFiveyears/fetch_IDCard') ?>",
        method: "POST",
        data: {
            IDCard: IDCard
        },
...
function fetch_IDCardEdit(count) {
    ...
    $.ajax({
        url: "<?= site_url('ConsiderFiveyears/fetch_IDCardEdit') ?>",
        method: "POST",
        data: {
            IDCard: IDCard
        },
...
function InsertConsiderFiveyears() {
    ...
    $.ajax({
        url: "<?= site_url('ConsiderFiveyears/InsertConsiderFiveyears') ?>",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function UpdateConsiderFiveyears(id, modalCount) {
    ...
    $.ajax({
        url: "<?= site_url('ConsiderFiveyears/update_ConsiderFiveyears') ?>",
        method: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function Delete_ConsiderFiveyears(id) {
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= site_url('ConsiderFiveyears/delete_ConsiderFiveyears') ?>",
                type: 'POST',
                data: {
                    id: id
                },
