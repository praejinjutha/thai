...
function get_PerformanceAppraisal() {
    ...
    $.ajax({
        url: "<?= site_url('PerformanceAppraisal/fech_PerformanceAppraisal') ?>",
        method: "POST",
        data: {
            Year: Year,
            FullName: FullName
            ...
            if (data.length === 0) {
                ...
                $.each(data, function(index, row) {
                    ...
                    let table_row = `<tr>
                            ...
                            <td>${row.Result || ''}</td>
                            <td>${row.Opinion || ''}</td>
                            <td><a href="<?= site_url('PerformanceAppraisal/export/') ?>${row.ID}">${row.FileName || ''}</a></td>
                                                ...
                                                <div class="modal-content">
                                                        <div class="modal-body" style="text-align: left">
                                                            <div class="tab">
                                                                <h5 class="card-title fw-bold"
                                                                    style="color: #18409e;font-size: 32px;">
                                                                    แก้ไขข้อมูลการประเมินผลการปฏิบัติงาน</h5>
                                                            </div>
                                                            <div id="modal-setting-7" class="tabcontent">
                                                            <hr style="color: #eac178;height: 3px;opacity: 1;">
                                                                <div class="container">
                                                                    ...
                                                                    <div class="row">
                                                                        ...
                                                                        <div class="col">
                                                                            <div class="form-group mt-2" id="FileNameDownload${count}">
                                                                                ...
                                                                                <a href="<?= site_url('PerformanceAppraisal/export/') ?>${row.ID}"><input class="form-control" type="text" id="FileNameEdit${count}" name="FileNameEdit" value="${row.FileName !== null ? row.FileName : ''}" style="cursor: pointer; color: blue;" readonly></a>
                                                                            </div>
                            ...
                            <td><a href="<?= site_url('PerformanceAppraisal/exportExcel/') ?>${row.ID}" class="btn btn-primary fw-bold fs-4">ส่งออก</a></td>
                        </tr>`;
                    table_body.append(table_row);
                });
            }
...
function fetch_Appraisee() {
    ...
    $.ajax({
        url: "<?= site_url('PerformanceAppraisal/fetch_Appraisee') ?>",
        method: "POST",
        data: {
            IDCard: IDCard
        },
...
function fetch_AppraiseeEdit(count) {
    ...
    $.ajax({
        url: "<?= site_url('PerformanceAppraisal/fetch_AppraiseeEdit') ?>",
        method: "POST",
        data: {
            IDCard: IDCard
        },
...
function InsertPerformanceAppraisal() {
    ...
    $.ajax({
        url: "<?= site_url('PerformanceAppraisal/Insert_PerformanceAppraisal') ?>",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function UpdatePerformanceAppraisal(ID, modalCount) {
    ...
    $.ajax({
        url: "<?= site_url('PerformanceAppraisal/update_PerformanceAppraisal') ?>",
        method: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function DeletePerformanceAppraisal(ID) {
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= site_url('PerformanceAppraisal/delete_PerformanceAppraisal') ?>",
                type: 'POST',
                data: {
                    ID: ID
                },
