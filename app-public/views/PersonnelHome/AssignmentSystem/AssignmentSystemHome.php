...
function get_AssignmentSystem() {
    ...
    $.ajax({
        url: "<?= site_url('AssignmentSystem/fech_AssignmentSystem') ?>",
        method: "POST",
        data: {
            Year: Year
        },
            ...
            if (data.length === 0) {
                ...
                $.each(data, function(index, row) {
                    ...
                    let table_row = `<tr>
                            ...
                            <td>${row.TeaAll || ''}</td>
                            <td><a href="<?= site_url('AssignmentSystem/export/') ?>${row.IdAt}">${row.FileNm || ''}</a></td>
                            <td>${row.Prother || ''}</td>
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="modal-body" style="text-align: left">
                                                            <div class="tab">
                                                                <h5 class="card-title fw-bold"
                                                                    style="color: #18409e;font-size: 32px;">
                                                                    แก้ไขข้อมูลการมอบหมายงาน / ติดตามงาน</h5>
                                                            </div>
                                                            <div id="modal-setting-7" class="tabcontent">
                                                                <hr style="color: #eac178;height: 3px;opacity: 1;">
                                                                <div class="container">
                                                                    ...
                                                                    <div class="row">
                                                                        ...
                                                                        <div class="col">
                                                                            <div class="form-group mt-2" id="FileNameDownload${count}">
                                                                                ...
                                                                                <a href="<?= site_url('AssignmentSystem/export/') ?>${row.IdAt}"><input class="form-control" type="text" id="FileNameEdit${count}" name="FileNameEdit" value="${row.FileNm !== null ? row.FileNm : ''}" style="cursor: pointer; color: blue;" disabled></a>
                                                                            </div>
                            ...
                            <td><a href="<?= site_url('AssignmentSystem/Export_Data/')?>${row.IdAt}" class="btn btn-primary fw-bold fs-4">ส่งออก</a></td>
                        </tr>`;
                    table_body.append(table_row);
                });
            }
...
function InsertAssignmentSystem() {
    ...
    $.ajax({
        url: "<?= site_url('AssignmentSystem/Insert_AssignmentSystem') ?>",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function UpdateAssignmentSystem(IdAt, modalCount) {
    ...
    $.ajax({
        url: "<?= site_url('AssignmentSystem/update_AssignmentSystem') ?>",
        method: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function Delete_AssignmentSystem(IdAt) {
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= site_url('AssignmentSystem/delete_AssignmentSystem') ?>",
                type: 'POST',
                data: {
                    IdAt: IdAt
                },
