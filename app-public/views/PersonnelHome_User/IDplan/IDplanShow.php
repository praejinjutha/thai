...
function get_PerformanceAgreement() {
    ...
    $.ajax({
        url: "<?= site_url('User_Controller/IDplan/fech_PerformanceAgreement') ?>",
        method: "POST",
        data: {
            IDCard: IDCard,
            Year: Year
            ...
            if (data.length === 0) {
                ...
                $.each(data, function(index, row) {
                    ...
                    <td>${count}</td>
                            ...
                            <td>${row.Topic || ''}</td>
                            <td><a href="<?= site_url('User_Controller/IDplan/export/') ?>${row.id}">${row.FileName || ''}</a></td>
                            <td>${row.note || ''}</td>
                            <td>${row.Status || ''}</td>
                                                        ...
                                                        <div class="row">
                                                            <div class="col">
                                                                <div class="form-group mt-2" id="FileNameDownload${count}">
                                                                    ...
                                                                    <a href="<?= site_url('User_Controller/IDplan/export/') ?>${row.id}"><input class="form-control" type="text" id="FileNameEdit${count}" name="FileNameEdit" value="${row.FileName !== null ? row.FileName : ''}" style="cursor: pointer; color: blue;" readonly></a>
                                                                </div>
...
function InsertPA1() {
    ...
    $.ajax({
        url: "<?= site_url('User_Controller/IDplan/Insert_PerformanceAgreement') ?>",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function UpdateIDplan(id, modalCount) {
    ...
    $.ajax({
        url: "<?= site_url('User_Controller/IDplan/update_IDplan') ?>",
        method: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function Delete_IDplan(id) {
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= site_url('User_Controller/IDplan/delete_IDplan') ?>",
                type: 'POST',
                data: {
                    id: id
                },
