...
function get_RequestForInsignia() {
    ...
    $.ajax({
        url: "<?= site_url('User_Controller/RequestForInsignia/fech_RequestForInsignia') ?>",
        method: "POST",
        data: {
            Year: Year
        },
            ...
            if (data.length === 0) {
                ...
                $.each(data, function(index, row) {
                    ...
                    let table_row = `<tr>
                            ...
                            <td><a href="<?= site_url('User_Controller/RequestForInsignia/export/') ?>${row.ID}">${row.FileName || ''}</a></td>
                                                    ...
                                                    <div class="row">
                                                        <div class="col">
                                                            ...
                                                            <div class="form-group mt-2" id="FileNameDownload${count}">
                                                                ...
                                                                <a href="<?= site_url('User_Controller/RequestForInsignia/export/') ?>${row.ID}"><input class="form-control" type="text" id="FileNameEdit${count}" name="FileNameEdit" value="${row.FileName !== null ? row.FileName : ''}" style="cursor: pointer; color: blue;" readonly></a>
                                                            </div>
                            ...
                            <td><a href="<?= site_url('User_Controller/RequestForInsignia/Export_Data/')?>${row.ID}" class="btn btn-primary fw-bold fs-4">ส่งออก</a></td>
                        </tr>`;
                    table_body.append(table_row);
                });
            }
...
function InsertRequestForInsignia() {
    ...
    $.ajax({
        url: "<?= site_url('User_Controller/RequestForInsignia/Insert_RequestForInsignia') ?>",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function UpdateRequestForInsignia(ID, modalCount) {
    ...
    $.ajax({
        url: "<?= site_url('User_Controller/RequestForInsignia/update_RequestForInsignia') ?>",
        method: "POST",
        data: formData,
        processData: false,
        contentType: false,
...
function FrmRequestForInsignia(ID) {
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: "<?= site_url('User_Controller/RequestForInsignia/delete_RequestForInsignia') ?>",
                type: 'POST',
                data: {
                    ID: ID
                },
